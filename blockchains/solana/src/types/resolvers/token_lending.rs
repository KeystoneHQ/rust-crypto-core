    ...
    match instruction {
        LendingInstruction::InitLendingMarket { owner, quote_currency } => {
            ...
            let lending_market_account = accounts.get(0).ok_or(SolanaError::AccountNotFound(format!(
                "{}.lending_market_account", method_name,
            )))?;
        ...
        LendingInstruction::InitReserve { liquidity_amount, config } => {
            ...
            let destination_collateral_pubkey = accounts.get(1).ok_or(SolanaError::AccountNotFound(format!(
                "{}.current_owner", method_name
            )))?;
            let reserve_pubkey = accounts.get(2).ok_or(SolanaError::AccountNotFound(format!(
                "{}.reserve_liquidity_mint_pubkey", method_name
            )))?;
            let reserve_liquidity_mint_pubkey = accounts.get(3).ok_or(SolanaError::AccountNotFound(format!(
                "{}.current_owner", method_name
            )))?;
            let reserve_liquidity_supply_pubkey = accounts.get(4).ok_or(SolanaError::AccountNotFound(format!(
                "{}.current_owner", method_name
            )))?;
            let reserve_liquidity_fee_receiver_pubkey = accounts.get(5).ok_or(SolanaError::AccountNotFound(format!(
                "{}.current_owner", method_name
            )))?;
            let reserve_collateral_mint_pubkey = accounts.get(6).ok_or(SolanaError::AccountNotFound(format!(
                "{}.current_owner", method_name
            )))?;
            let reserve_collateral_supply_pubkey = accounts.get(7).ok_or(SolanaError::AccountNotFound(format!(
                "{}.current_owner", method_name
            )))?;
            ...
            let lending_market_pubkey = accounts.get(10).ok_or(SolanaError::AccountNotFound(format!(
                "{}.current_owner", method_name
            )))?;
            let lending_market_authority_pubkey = accounts.get(11).ok_or(SolanaError::AccountNotFound(format!(
                "{}.current_owner", method_name
            )))?;
            let lending_market_owner_pubkey = accounts.get(12).ok_or(SolanaError::AccountNotFound(format!(
                "{}.current_owner", method_name
            )))?;
            let user_transfer_authority_pubkey = accounts.get(13).ok_or(SolanaError::AccountNotFound(format!(
                "{}.current_owner", method_name
            )))?;
            let sysvar_clock = accounts.get(14).ok_or(SolanaError::AccountNotFound(format!(
                "{}.current_owner", method_name
            )))?;
            let sysvar_rent = accounts.get(15).ok_or(SolanaError::AccountNotFound(format!(
                "{}.current_owner", method_name
            )))?;
            let token_program_id = accounts.get(16).ok_or(SolanaError::AccountNotFound(format!(
                "{}.current_owner", method_name
            )))?;
        ...
        LendingInstruction::RefreshReserve => {
            ...
            let reserve_account = accounts.get(0).ok_or(SolanaError::AccountNotFound(format!(
                "{}.reserve_account", method_name,
            )))?;
            let reserve_liquidity_oracle_account = accounts.get(1).ok_or(SolanaError::AccountNotFound(format!(
                "{}.reserve_liquidity_oracle_account", method_name
            )))?;
            let sysvar_clock = accounts.get(1).ok_or(SolanaError::AccountNotFound(format!(
                "{}.sysvar_clock", method_name
            )))?;
            Ok(template_instruction(
                program_name,
                method_name,
        ...
        LendingInstruction::DepositReserveLiquidity { liquidity_amount } => {
            ...
            let source_liquidity_pubkey = accounts.get(0).ok_or(SolanaError::AccountNotFound(format!(
                "{}.source_liquidity_pubkey", method_name,
            )))?;
            let destination_collateral_pubkey = accounts.get(1).ok_or(SolanaError::AccountNotFound(format!(
                "{}.destination_collateral_pubkey", method_name
            )))?;
            let reserve_pubkey = accounts.get(2).ok_or(SolanaError::AccountNotFound(format!(
                "{}.reserve_pubkey", method_name
            )))?;
            let reserve_liquidity_supply_pubkey = accounts.get(3).ok_or(SolanaError::AccountNotFound(format!(
                "{}.reserve_liquidity_supply_pubkey", method_name
            )))?;
            let reserve_collateral_mint_pubkey = accounts.get(4).ok_or(SolanaError::AccountNotFound(format!(
                "{}.reserve_collateral_mint_pubkey", method_name
            )))?;
            let lending_market_pubkey = accounts.get(5).ok_or(SolanaError::AccountNotFound(format!(
                "{}.lending_market_pubkey", method_name
            )))?;
            let lending_market_authority_pubkey = accounts.get(6).ok_or(SolanaError::AccountNotFound(format!(
                "{}.lending_market_authority_pubkey", method_name
            )))?;
            let user_transfer_authority_pubkey = accounts.get(7).ok_or(SolanaError::AccountNotFound(format!(
                "{}.user_transfer_authority_pubkey", method_name
            )))?;
        ...
        LendingInstruction::RedeemReserveCollateral { collateral_amount } => {
            ...
            let source_collateral_pubkey = accounts.get(0).ok_or(SolanaError::AccountNotFound(format!(
                "{}.source_collateral_pubkey", method_name,
            )))?;
            let destination_liquidity_pubkey = accounts.get(1).ok_or(SolanaError::AccountNotFound(format!(
                "{}.destination_liquidity_pubkey", method_name
            )))?;
            let reserve_pubkey = accounts.get(2).ok_or(SolanaError::AccountNotFound(format!(
                "{}.reserve_pubkey", method_name
            )))?;
            let reserve_collateral_mint_pubkey = accounts.get(3).ok_or(SolanaError::AccountNotFound(format!(
                "{}.reserve_collateral_mint_pubkey", method_name
            )))?;
            let reserve_liquidity_supply_pubkey = accounts.get(4).ok_or(SolanaError::AccountNotFound(format!(
                "{}.reserve_liquidity_supply_pubkey", method_name
        ...
        LendingInstruction::InitObligation => {
            ...
            let obligation_account = accounts.get(0).ok_or(SolanaError::AccountNotFound(format!(
                "{}.obligation_account", method_name,
            )))?;
        ...
        LendingInstruction::DepositObligationCollateral { collateral_amount } => {
            ...
            let obligation_owner_pubkey = accounts.get(5).ok_or(SolanaError::AccountNotFound(format!(
                "{}.obligation_owner_pubkey", method_name
            )))?;
            let user_transfer_authority_pubkey = accounts.get(6).ok_or(SolanaError::AccountNotFound(format!(
                "{}.user_transfer_authority_pubkey", method_name
            )))?;
        ...
        LendingInstruction::WithdrawObligationCollateral { collateral_amount } => {
            ...
            let lending_market_authority_pubkey = accounts.get(5).ok_or(SolanaError::AccountNotFound(format!(
                "{}.lending_market_authority_pubkey", method_name
            )))?;
        ...
        LendingInstruction::BorrowObligationLiquidity { liquidity_amount } => {
            ...
            let borrow_reserve_pubkey = accounts.get(2).ok_or(SolanaError::AccountNotFound(format!(
                "{}.borrow_reserve_pubkey", method_name
            )))?;
            let borrow_reserve_liquidity_fee_receiver_pubkey = accounts.get(3).ok_or(SolanaError::AccountNotFound(format!(
                "{}.borrow_reserve_liquidity_fee_receiver_pubkey", method_name
            )))?;
        ...
        LendingInstruction::RepayObligationLiquidity { liquidity_amount } => {
            ...
            let repay_reserve_pubkey = accounts.get(2).ok_or(SolanaError::AccountNotFound(format!(
                "{}.repay_reserve_pubkey", method_name
            )))?;
            let obligation_pubkey = accounts.get(3).ok_or(SolanaError::AccountNotFound(format!(
                "{}.obligation_pubkey", method_name
            )))?;
            ...
            let user_transfer_authority_pubkey = accounts.get(5).ok_or(SolanaError::AccountNotFound(format!(
                "{}.lending_market_authority_pubkey", method_name
            )))?;
        ...
        LendingInstruction::LiquidateObligation { liquidity_amount } => {
            ...
            let repay_reserve_liquidity_supply_pubkey = accounts.get(3).ok_or(SolanaError::AccountNotFound(format!(
                "{}.repay_reserve_liquidity_supply_pubkey", method_name
            )))?;
            let withdraw_reserve_pubkey = accounts.get(4).ok_or(SolanaError::AccountNotFound(format!(
                "{}.withdraw_reserve_pubkey", method_name
            )))?;
            let withdraw_reserve_collateral_supply_pubkey = accounts.get(5).ok_or(SolanaError::AccountNotFound(format!(
                "{}.withdraw_reserve_collateral_supply_pubkey", method_name
            )))?;
            ...
            let lending_market_authority_pubkey = accounts.get(8).ok_or(SolanaError::AccountNotFound(format!(
                "{}.lending_market_authority_pubkey", method_name
            )))?;
            let user_transfer_authority_pubkey = accounts.get(9).ok_or(SolanaError::AccountNotFound(format!(
                "{}.user_transfer_authority_pubkey", method_name
            )))?;
            let sysvar_clock = accounts.get(10).ok_or(SolanaError::AccountNotFound(format!(
                "{}.sysvar_clock", method_name
            )))?;
            let token_program_id = accounts.get(11).ok_or(SolanaError::AccountNotFound(format!(
                "{}.token_program_id", method_name
            )))?;
        ...
        LendingInstruction::FlashLoan { amount } => {
            ...
            let reserve_liquidity_fee_receiver_pubkey = accounts.get(3).ok_or(SolanaError::AccountNotFound(format!(
                "{}.reserve_liquidity_fee_receiver_pubkey", method_name
            )))?;
            let host_fee_receiver_pubkey = accounts.get(4).ok_or(SolanaError::AccountNotFound(format!(
                "{}.host_fee_receiver_pubkey", method_name
            )))?;
            let lending_market_pubkey = accounts.get(5).ok_or(SolanaError::AccountNotFound(format!(
                "{}.lending_market_pubkey", method_name
            )))?;
            ...
            let flash_loan_receiver_program_id = accounts.get(8).ok_or(SolanaError::AccountNotFound(format!(
                "{}.flash_loan_receiver_program_id", method_name
            )))?;
