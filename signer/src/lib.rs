...
pub use crate::keymaster::{RSASignType, SigningOption};
use error::KSError;
pub use keymaster::SigningAlgorithm;
use keymaster::{local::Mini, se::SecureElement, KeyMaster};
...
impl Signer {
    ...
    pub fn get_rsa_public_key(
        ...
        self.inner.get_rsa_public_key(mnemonic_id, password)
    }
    ...
    pub fn setup_ada_root_key(
        ...
        self.inner
            .setup_ada_root_key(mnemonic_id, password, passphrase)
    }
    ...
    pub fn get_ada_extended_public_key(
        ...
        self.inner
            .get_ada_extended_public_key(mnemonic_id, password, path)
    }

    pub fn get_ada_root_key(&self, mnemonic_id: u8, password: String) -> Result<Vec<u8>, KSError> {
        self.inner.get_ada_root_key(mnemonic_id, password.into())
    }
}
...
mod tests {
    ...
    fn it_should_pass_test_sign_rsa_salt_zero() {
        ...
        let signing_option = SigningOption::RSA {
            salt_len: 0,
            sign_type: RSASignType::Common,
        };
    ...
    fn it_should_pass_test_sign_rsa_salt_digest_size() {
        ...
        let signing_option = SigningOption::RSA { salt_len: 32, sign_type: RSASignType::Common };
        ...
        let result = rsa.verify(
            &signature.as_ref(),
            &data2,
            SigningOption::RSA { salt_len: 32, sign_type: RSASignType::Common },
        );
        assert_eq!(result.ok(), Some(()));
    }
}
