...
use bytes::BytesMut;
use ed25519_bip32::DerivationIndex;
...
impl KeyMaster for SecureElement {
        ...
        match algo {
            SigningAlgorithm::RSA => {
                if let Some(password) = password {
                    let auth_token = hex::decode(password.clone())
                        .map_err(|_e| KSError::SEError("".to_string()))?;
                    ...
                    let secret = algorithm::rsa::RSA::from_seed(zeroize_master_seed.as_slice())
                        .map_err(|_| {
                    ...
                    let public_key_bytes = rsa.keypair_modulus().map_err(|_| {
                        KSError::GenerateSigningKeyError("get rsa public key failed".to_string())
                    })?;
                    public_key.extend_from_slice(&public_key_bytes);
                    Ok(public_key.to_vec())
                } else {
                    Err(KSError::GetPublicKeyError(
                        "password is required".to_string(),
                    ))
                }
            }
            _ => Err(KSError::GetPublicKeyError(
                "get public key algorithm is not supported yet".to_string(),
            )),
        }
    }
    ...
    fn derive(
        ...
        auth_token: Option<Vec<u8>>,
        algo: SigningAlgorithm,
        derivation_algo: super::KeyDerivationAlgorithm,
        derivation_path: String,
    ) -> Result<Vec<u8>, KSError> {
        let key = self.get_key(mnemonic_id, derivation_path.clone(), auth_token, algo.clone(), GetKeyType::MasterSeed)?;
        let bip39passphrase = vec![]; // should get from se;
        if let (SigningAlgorithm::Ed25519, KeyDerivationAlgorithm::Bip32Ed25519) =
            (algo, derivation_algo)
        {
            let result = key_derivation::bip32_ed25519::derive(&key, &bip39passphrase, &derivation_path)?;
            return Ok(result);
        }
        Err(KSError::KeyDerivationError(format!(
            "only supports bip32-ed25519 currently"
        )))
    }
}
...
mod tests {
    ...
    fn it_should_test_get_rsa_public_key() {
        ...
        let key = se
            .get_public_key(0, Some(password), SigningAlgorithm::RSA, Some(path))
            .unwrap();
        se.clear_token();
