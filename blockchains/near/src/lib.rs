...
impl Near {
    fn parse_formatted_data(data: &Vec<u8>) -> Result<String, NearError> {
        let tx = Parser::parse(data)?;
        tx.get_formatted_json()
    }
}
...
mod tests {
    ...
    fn test() {
        ...
        assert_eq!(json, Near::parse_formatted_data(&mut buf_message).unwrap_or_else(|e| e.to_string()));
    }
}
